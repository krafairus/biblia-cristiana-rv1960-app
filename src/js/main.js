...
class App {
  ...
  setupHardwareBackButton() {
    if (this.capacitorApp) {
      this.capacitorApp.addListener('backButton', ({ canGoBack }) => {
  ...
  handleHardwareBack() {
    ...
    // 2. Editor sub-panels
    const activePanels = document.querySelectorAll('.editor-inline-panel.active, #toolbar-secondary-row, #toolbar-header-row, #toolbar-more-row');
    let panelClosed = false;
    activePanels.forEach(p => {
        if(p.style.display !== 'none' && !p.classList.contains('active') && p.id.startsWith('toolbar')) {
    ...
    // 3. Selection toolbars
    const selectionBar = document.getElementById('selection-bar');
    const highlightBar = document.getElementById('highlight-bar');
    if ((selectionBar && selectionBar.style.display !== 'none') || (highlightBar && highlightBar.style.display !== 'none')) {
      this.clearSelection();
      return;
    }
    ...
    if (ttsDialog && ttsDialog.style.display !== 'none') {
      if (this.closeTTSDialog) this.closeTTSDialog();
      return;
    }
    ...
    switch (this.currentView) {
      ...
      case 'about':
        ...
        break;
      case 'chapters':
        if (this.currentBook && this.db) {
          const oldBooks = this.db.getBooks('old');
          const isOld = oldBooks.includes(this.currentBook);
          this.renderBookList(isOld ? 'old' : 'new');
        } else {
          this.navigate('home');
        }
        break;
      case 'verses':
        if (this.currentBook) {
          this.renderChapterList(this.currentBook);
        } else {
          this.navigate('home');
        }
        break;
      case 'reader':
        if (this.currentBook && this.currentChapter) {
          this.renderVerseList(this.currentBook, this.currentChapter);
        ...
        break;
      case 'note-editor':
        if (this.cancelNoteEditor) this.cancelNoteEditor();
      ...
      case 'devocional':
      case 'preguntas':
      case 'devotional-favorites':
      case 'devotional-history':
      case 'preguntas-history':
      case 'devotional-detail':
      case 'preguntas-detail':
      case 'editor-admin':
  ...
  async init() {
    ...
    if (this.isElectron) {
      document.body.classList.add('electron');
      // this.setupTitleBarEvents(); // Solo si existe el método
    }
  }
  ...
  renderNoteEditor(index = null, source = 'notes') {
        ...
        <div style="flex:1; min-height:0; position:relative; margin-top:0.5rem; display:flex; flex-direction:column;">
          ...
          <div id="editor-scroll-container" style="flex:1; min-height:0; overflow-y:auto; overflow-x:hidden; position:relative; -webkit-overflow-scrolling:touch; padding-top:0.25rem;">
            <div id="editor-note-text" class="rich-editor" contenteditable="true"
                 ...
                 style="min-height:200px; outline:none; padding:0 1.25rem;">${n.notes || n.note || ''}</div>
    ...
    const _ensureCaretVisible = () => {
      const vp = window.visualViewport;
      if (!vp) return;
      // We assume keyboard is visible if viewport is somewhat small
      const isKeyboardVisible = (window.innerHeight - vp.height) > 60 || vp.height < window.screen.height * 0.7;
      ...
      if (sel && sel.rangeCount > 0) {
         try {
           ...
           // Sometimes empty lines have 0-width rects but valid top/bottom
           let rect = range.getBoundingClientRect();
           
           // If rect is 0 (can happen on empty new lines), try creating a temp node
           ...
           if (rect.bottom > 0 && rect.bottom > vp.height - 65) {
             const scrollContainer = document.getElementById('editor-scroll-container');
             // Solo hace scroll si está explícitamente debajo del umbral visible
             if (scrollContainer) scrollContainer.scrollBy({ top: rect.bottom - (vp.height - 65), behavior: 'smooth' });
           }
         } catch(e){}
      }
    };

    let _wasKeyboardVisible = false;
    const _updateToolbarPos = () => {
      const vp = window.visualViewport;
      if (!vp) return;

      const keyboardHeight = window.innerHeight - vp.height;
      const isKeyboardVisible = keyboardHeight > 60 || vp.height < window.screen.height * 0.7;

      // Resize the main container so the flex layout correctly sizes the wrapper
      if (this.appEl) {
        this.appEl.style.height = vp.height + 'px';
      }

      // Ajustar barra de herramientas
      if (toolbarWrap) {
        toolbarWrap.style.bottom = '0px'; // Android webview resizes, fixed bottom:0 is enough
        if (isKeyboardVisible) {
          toolbarWrap.style.paddingBottom = '0.5rem';
        } else {
          toolbarWrap.style.paddingBottom = '1.25rem'; 
        }
      }
      ...
      // Auto-scroll ONLY when the keyboard transitions from closed to open
      if (isKeyboardVisible && !_wasKeyboardVisible) {
         _wasKeyboardVisible = true;
         setTimeout(() => _ensureCaretVisible(), 100);
    ...
    if (editor) {
      ...
      editor.addEventListener('input', () => {
        _updateToolbarState();
        setTimeout(() => _ensureCaretVisible(), 50);
      });
  ...
  cancelNoteEditor() {
    ...
    const noteInput = document.querySelector('#editor-note-text');
    const title = titleInput ? titleInput.value.trim() : '';
    const note = noteInput ? noteInput.innerText.trim() : '';
    
    const hasContent = title.length > 0 || note.length > 0;
    ...
    if (hasContent) {
      this.openConfirmModal(
        "¿Salir del editor?",
        "¿Estás seguro de que deseas salir del editor? Si tienes cambios sin guardar, se perderán.",
        executeExit,
        "Salir",
        "#ef4444"
      );
  ...
  async renderGenericHistoryView(title, icon, indexFile, type) {
    ...
    try {
      ...
      items.sort((a, b) => {
        const id1 = a.id || 0;
        const id2 = b.id || 0;
        return sortOrder === 'asc' ? id1 - id2 : id2 - id1;
      });
